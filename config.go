...
type widgetPositionAssignment struct {
	Widget wallpaperProvider   `json:"widget"`
	Corner weatherWidgetCorner `json:"corner"`
}
...
type monitorProviderAssignment struct {
	...
	WidgetPositions []widgetPositionAssignment `json:"widgetPositions,omitempty"`
	StackWidgets    bool                       `json:"stackWidgets"`
}
...
type appConfig struct {
	...
	KanbanTheme                  string                      `json:"kanbanTheme"`
...
func (c appConfig) normalized() appConfig {
	...
	if clone.KanbanTheme == "" {
		clone.KanbanTheme = "default"
	}
...
func (c appConfig) toRuntimeClientConfig(providers []wallpaperProvider, monitorIndex int, weatherBackgroundImageURL string) runtimeClientConfig {
	var widgetPositions []widgetPositionAssignment
	stackWidgets := false
	for _, assignment := range c.MonitorAssignments {
		if assignment.MonitorIndex == monitorIndex {
			widgetPositions = assignment.WidgetPositions
			stackWidgets = assignment.StackWidgets
			break
		}
	}
