...
export function IssueCard({ issue }: IssueCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Footer: start date and due date */}
      {(issue.start_date || issue.target_date) && (
        <div 
          className="flex items-center justify-end gap-1.5 text-[10px] select-none mt-1 shrink-0 font-medium"
          style={{ color: "var(--plane-text-muted)" }}
        >
          ...
          <div className="flex items-center gap-1">
            ...
            {issue.start_date && issue.target_date && <span className="opacity-50">→</span>}
            {issue.target_date && (
              <span style={{ color: overdueTarget ? "#ef4444" : "inherit" }}>
