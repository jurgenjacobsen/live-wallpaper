...
export function WidgetWallpaper({ runtimeConfig, onInitialDataReady }: WidgetWallpaperProps) {
  ...
  // Map widgets to active corners
  const getWidgetsForCorner = (corner: 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right') => {
    const list: ('weather' | 'currency')[] = [];
    
    // Resolve weather corner
    const weatherPos = runtimeConfig.widgetPositions?.find(p => p.widget === 'weather');
    const weatherCorner = weatherPos ? weatherPos.corner : runtimeConfig.weather.corner || 'top-right';
    if (hasWeather && weatherCorner === corner) {
      list.push('weather');
    }

    // Resolve currency corner
    const currencyPos = runtimeConfig.widgetPositions?.find(p => p.widget === 'currency');
    const currencyCorner = currencyPos ? currencyPos.corner : runtimeConfig.weather.corner || 'top-right';
    if (hasCurrency && currencyCorner === corner) {
      list.push('currency');
    }
  ...
  return (
    ...
    >
      ...
      {/* Render each corner containing widgets */}
      {corners.map(corner => {
        const widgetsInCorner = getWidgetsForCorner(corner);
        if (widgetsInCorner.length === 0) return null;
        ...
        return (
          ...
          >
            {widgetsInCorner.includes('weather') && (
              <div
                style={{
                  padding: "12px",
                  borderRadius: "14px",
            ...
            {widgetsInCorner.includes('currency') && (
              <div
                style={{
                  padding: "12px",
                  borderRadius: "14px",
